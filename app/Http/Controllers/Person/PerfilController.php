...
class PerfilController extends Controller
{
    ...
    public function edit($id)
    {
        ...
        $dato = $this->gen_section();        
        return view('person.perfil.edit',compact('perfil','dato'));
    }
    
    public function editcredentials($id)
    {
        $perfil = Person::findOrFail($id);
        $perfil["password"] = "";
        $this->genLog("Ingresó a editar credenciales email: ".$perfil->email);
        $dato = $this->gen_section();        

        return view('person.perfil.editcredentials',compact('perfil','dato'));
    }
    ...
    public function updatecredentials(Request $request, $id)
    {
        ...
        try {       
            ...
            $perfil = Person::findOrFail($id);
            $perfil->update($requestData);
            Session::flash('flash_message', 'Actualizado credenciales correctamente');
            $this->genLog("Actualizó cuenta email : ".$perfil->email);
            ...
            Session::flash('warning', 'Error al actualizar!!!');            
            $this->genLog("Error al actualizar credenciales : ".$perfil->email);
        }
    ...
    public function gen_section(){
        $data = array(
            "txtinicio"=>"Inicio",
            "rutainicio" => "/person/settings",
            "txtmodulo" => "PERFÍL",
            "section" => "Perfil",
            "rutamodulo" => "/person/settings",
            "ventana" => "Perfil"
        );
        return $data;
    }
